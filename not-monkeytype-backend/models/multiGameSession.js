...
multiGameSessionSchema.statics.getGameSessionsByEmail = async function (email) {
    try {
        const multiGameSessions = await this.find({ "scores.email": email });
        return multiGameSessions;
    } catch (error) {
        console.error(`Error fetching multiplayer sessions by email ${email}:`, error);
        throw error;
    }
};
