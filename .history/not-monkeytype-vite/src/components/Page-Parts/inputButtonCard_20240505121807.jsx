...
const InputButtonCard = ({ Title, InputTitle, ButtonTitle, onButtonClick, onCancel }) => { // Add onCancel prop
  ...
  const handleClick = () => {
    const result = onButtonClick(true, verificationCode); // Return true and the input text when clicked
    onClose(); // Close the window after clicking "Verify"
    return result; // Return the result
  };

  const handleCancel = () => { // Handle cancel action
    const result = onCancel(false, verificationCode); // Return false and the input text when canceled
    onClose(); // Close the window after clicking "Close"
    return result; // Return the result
  };
  ...
  return (
    ...
    <button onClick={handleCancel} className="mx-6 my-4 text-white outline-none focus:outline-none">
