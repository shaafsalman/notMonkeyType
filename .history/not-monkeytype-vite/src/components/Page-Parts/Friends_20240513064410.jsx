...
const Friends = () => {
  ...
  const [index, setIndex] = useState(0); // Initialize battle index to 0
  ...
  const fetchFriendBattles = async (email) => {
    try {
      ...
      console.log("Friend battles response:", response.data); // Log the response data
  ...
  const determineWinner = () => {
    if (friendBattles && friendBattles.player1 && friendBattles.player1.email &&
        friendBattles.player2 && friendBattles.player2.email) {
  ...
  if (error || !friendBattles || Object.keys(friendBattles).length === 0) {
    return (
