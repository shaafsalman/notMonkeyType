...
const Scores = () => {
  ...
  <thead>
    <tr className="text-xl">
      ...
      <th className="py-2 text-left">Word Per Minute</th>
  ...
  <tbody>
    {userGameSessions.map((session, index) => (
      <tr key={index} className={index % 2 === 0 ? "" : ""}>
        ...
        <td className="py-2 text-left">{session.wpm}</td>
        <td className="py-2 text-left">{session.accuracy}%</td>
        <td className="py-2 text-left">{formatDate(session.createdAt)}</td>
        <td className="py-2 text-left">{session.sessionTime}</td>
        <td className="py-2 text-left">{session.score}</td>
      </tr>
    ))}
  </tbody>
</table>
