  ...
  <div className="px-6 py-4">
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-white">
        <thead>
          <tr>
            <th className="py-3 px-4 bg-indigo-600">Rank</th>
            <th className="py-3 px-4 bg-indigo-600">Username</th>
            <th className="py-3 px-4 bg-indigo-600">Word Per Minute</th>
            <th className="py-3 px-4 bg-indigo-600">Accuracy</th>
            <th className="py-3 px-4 bg-indigo-600">Date</th>
            <th className="py-3 px-4 bg-indigo-600">Time Interval</th>
            <th className="py-3 px-4 bg-indigo-600">Score</th>
          </tr>
        </thead>
        <tbody>
          {userGameSessions.map((session, index) => (
            <tr key={index} className={index % 2 === 0 ? 'bg-indigo-100' : 'bg-indigo-200'}>
              <td className="py-3 px-4">{index + 1}</td>
              <td className="py-3 px-4">{session.name}</td>
              <td className="py-3 px-4">{session.wpm}</td>
              <td className="py-3 px-4">{session.accuracy}%</td>
              <td className="py-3 px-4">{formatDate(session.createdAt)}</td>
              <td className="py-3 px-4">{session.sessionTime}</td>
