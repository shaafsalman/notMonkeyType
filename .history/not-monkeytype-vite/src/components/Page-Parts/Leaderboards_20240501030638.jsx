...
const Leaderboards = () => {
  ...
  return (
    <div className="leaderboard-container">
      <div className="bg-gradient-to-br from-purple-500 to-indigo-600 rounded-lg shadow-lg p-6">
        <div className="text-4xl font-bold text-white mb-4">Leaderboard</div>
          ...
          <table className="w-full text-white">
            ...
            <tbody>
              {leaderboardData.map((data) => (
                <tr key={data.rank}>
                  <td className="py-2 px-2">{data.rank}</td>
                  <td className="py-2 px-4">{data.username}</td>
                  <td className="py-2 px-4">{data.score}</td>
                </tr>
              ))}
            </tbody>
          </table>
