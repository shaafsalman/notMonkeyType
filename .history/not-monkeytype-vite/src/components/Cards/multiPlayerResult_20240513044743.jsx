        ...
        <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
            <div className="max-w-screen-lg mx-4 rounded-lg overflow-hidden shadow-lg bg-gradient-to-br from-purple-500 to-indigo-600 transition duration-500 transform hover:scale-105">
                <div className="p-6 sm:p-8 lg:p-10">
                    <div className="text-white text-4xl font-bold mb-8 text-center">Results</div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {combinedScores.map((score, index) => (
                            <div key={index} className="bg-white rounded-lg shadow-md p-6 flex flex-col justify-between transition duration-300 ease-in-out transform hover:scale-105">
                                <div>
                                    <h3 className="text-2xl font-bold flex items-center mb-4 text-indigo-600">
                                    ...
                                    <p className="text-lg mt-2"><span className="font-semibold text-indigo-600">Email:</span> {score.email}</p>
                                    <p className="text-lg"><span className="font-semibold text-indigo-600">WPM:</span> {score.wpm.toFixed(2)}</p>
                                    <p className="text-lg"><span className="font-semibold text-indigo-600">Accuracy:</span> {score.accuracy.toFixed(2)}%</p>
                                </div>
                                <p className="text-lg mt-6"><span className="font-semibold text-indigo-600">Combined Score:</span> {score.combinedScore.toFixed(2)}</p>
                            </div>
                        ))}
                    </div>
                </div>
