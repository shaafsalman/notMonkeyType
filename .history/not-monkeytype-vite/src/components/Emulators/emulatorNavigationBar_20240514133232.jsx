...
const NavigationBar = ({ isMobile, handleDurationChange, testDuration, testStarted, startTest, endTest, mode, roomCode, setDifficultyLevel}) => {
  ...
  return (
    <nav className={`navbar ${isMobile ? 'flex flex-col items-center justify-center  w-max' : 'flex justify-between items-center px-10 py-4'} bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg shadow-lg transition duration-300`}>
      {!isMobile && (
        <div className="div">
           <div className="flex">
         <div className="flex items-center">
          ...
          <h1 className="mx-10 flex-grow text-xl md:text-2xl font-bold">notMonkeyType</h1>
            ...
            {roomCode&&  
            <div className="room-code mx-20">
              Room Code: {roomCode}
            </div>}
           
            <div className="mode mx-20">
              Mode: {mode}
            </div>
             </div>
            }
      ...
      {isMobile && (
        <div className="flex flex-col items-center h-60 w-96">
          <Link to="/home" className=" py-2 w-60 bg-indigo-800 hover:bg-indigo-900 text-white rounded-lg transition duration-300 text-center">
            Back
          </Link>
