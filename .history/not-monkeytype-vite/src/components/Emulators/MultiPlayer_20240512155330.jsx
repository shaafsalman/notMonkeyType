          ...
          <div className="w-1/2 px-11 py-40 mr-30">
            ...
            <div className="flex w-max">
              <button onClick={handleEnterClick} className="w-max px-4 py-2 mr-2 bg-blue-500 text-white rounded hover:bg-blue-600">
                {showInput ? 'Cancel' : (roomCode ? 'Generate Room Code' : 'Create Room')}
              </button>
              <button onClick={handleSubmit} className="w-max px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
                Join Room
              </button>
            </div>
            {roomCode && !showInput && (
