...
const NavigationBar = ({ isMobile, handleDurationChange, testDuration, testStarted, startTest, endTest, mode, roomCode, level }) => {
  ...
  return (
    <nav className={`navbar ${isMobile ? 'flex flex-col items-center justify-center p-4' : 'flex justify-between items-center px-10 py-4'} bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg shadow-lg transition duration-300`}>
      ...
      <div className="level-slider">
        ...
        <input type="range" id="level" name="level" min="1" max="3" value={currentLevel} onChange={handleLevelChange} />
        <div className="level-buttons">
          <button className={`level-button ${currentLevel === 1 ? 'active' : ''}`} onClick={() => setCurrentLevel(1)}>Beginner</button>
          <button className={`level-button ${currentLevel === 2 ? 'active' : ''}`} onClick={() => setCurrentLevel(2)}>Intermediate</button>
          <button className={`level-button ${currentLevel === 3 ? 'active' : ''}`} onClick={() => setCurrentLevel(3)}>Expert</button>
        </div>
      </div>
    </nav>
  );
