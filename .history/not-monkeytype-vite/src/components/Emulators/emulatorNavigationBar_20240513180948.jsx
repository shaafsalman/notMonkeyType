  ...
  return (
    <nav className={`navbar ${isMobile ? 'flex flex-col items-center justify-center p-4' : 'flex justify-between items-center px-10 py-4'} bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg shadow-lg transition duration-300`}>
      ...
      <div className="level-toggle">
        <button className={`toggle-button ${currentLevel === 'beginner' ? 'active' : ''}`} onClick={toggleLevel}>
          Beginner
        </button>
        <button className={`toggle-button ${currentLevel === 'intermediate' ? 'active' : ''}`} onClick={toggleLevel}>
          Intermediate
        </button>
        <button className={`toggle-button ${currentLevel === 'expert' ? 'active' : ''}`} onClick={toggleLevel}>
          Expert
        </button>
      </div>
    </nav>
