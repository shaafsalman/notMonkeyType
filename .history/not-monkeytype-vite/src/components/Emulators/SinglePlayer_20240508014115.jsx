...
const SinglePlayer = () => {
  ...
  useEffect(() => {
    let timer;
    if (testStarted && timeRemaining > 0 && !isTyping) {
      timer = setInterval(() => {
        setTimeRemaining((prev) => prev - 1);
      }, 1000);
    } else if ((timeRemaining === 0 || userInput === testText) && testStarted) {
      endTest();
    }
    return () => clearInterval(timer);
  }, [testStarted, timeRemaining, isTyping, userInput, testText]);
  ...
  const onInput = (e) => {
    ...
    if (value.trim() === '') {
      setIsTyping(false); // If empty, set typing status to false
    } else {
      setIsTyping(true); // If not empty, set typing status to true
    }
  };
