...
const ToggleComponent = ({ selectedValue, onToggleChange }) => {
  return (
    <div className="relative w-full mt-4 rounded-md border h-10 p-1 bg-transparent">
      ...
      <span className={`${selectedValue === 'beginner' ? 'left-1' : selectedValue === 'intermediate' ? 'left-1/3' : 'left-2/3'} text-gray-800 bg-white  text-md flex items-center justify-center w-1/3 rounded h-[1.88rem] transition-all duration-300 ease-linear top-[4px] absolute`}>
