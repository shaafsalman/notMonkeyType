  ...
  return (
    <div className="relative w-full mt-4 rounded-md border h-10 p-1 bg-gray-200">
      <div className="flex w-full">
        <div className={`w-1/3 flex justify-center items-center cursor-pointer ${selectedValue === 'beginner' ? 'bg-indigo-300' : 'bg-gray-200'}`} onClick={() => onToggleChange('beginner')}>
      ...
      <span className={`${selectedValue === 'beginner' ? 'left-1/3' : selectedValue === 'intermediate' ? 'left-1/2' : 'left-2/3'} -translate-x-1/2 transform -translate-y-full text-gray-800 bg-white shadow text-sm flex items-center justify-center w-1/3 rounded h-[1.88rem] transition-all duration-300 ease-linear top-[4px] absolute`}>
