...
const ToggleComponent = ({ selectedValue, onToggleChange }) => {
  return (
    <div className="relative w-full mt-4 rounded-md h-10 p-1  bg-transparent">
      ...
      <span className={`${selectedValue === 'beginner' ? 'left-1' : selectedValue === 'intermediate' ? 'left-1/3' : 'left-2/3'} text-white bg-indigo-800  lg:px-4 lg:py-4 lg:text-xl flex items-center justify-center w-1/3 rounded h-[1.88rem] transition-all duration-300 ease-linear top-[4px] absolute`}>
